...
public:
    ...
    vector<Edge> primMST(int startNode) {
        ...
        priority_queue<Edge, vector<Edge>, CompareEdge> pq;  // Priority queue to store edges
        vector<bool> inMST(V, false);                        // To track vertices already in MST
        vector<Edge> mstEdges;                               // To store the edges of MST

        inMST[startNode] = true;                             // Add starting node to MST

        // Add all edges connected to starting node to priority queue
        
        for (auto neighbor : adjList[startNode]) {
            // Edges aren't stored in the graph as the structure Edge,
            // so we need to create one for each neighbor and add to the queue
            // Neighbor is a vector<pair<int, int>> of dest and weight, and we access each by .first and .second
            pq.push(Edge(startNode, neighbor.first, neighbor.second));
        }

        while (!pq.empty()) {
            Edge e = pq.top(); // Get the minimum weight edge
            pq.pop(); // Has no return value, so we can't do Edge e = pq.pop();
            ...
            inMST[u] = true;  // You forgot this

            // Add all edges connected to u to priority queue
            for (auto neighbor : adjList[u]) {
                // You don't need to check if neighbour's already visited
...
int main() {
    ...
    int startNode = 3; // Starting node for Prim's algorithm
    ///cin >> startNode;
